...
- 仅在当前课程仓库内工作；不修改系统配置、Shell 配置、全局软件包、工具链或模拟器，除非任务明确授权。
- 不覆盖既有未提交改动；执行删除、`git clean`、`git restore` 等可能丢失数据的操作前，先确认目标和影响。
- 优先保护老师或助教提供的源码；除非任务明确要求，不主动重构、删除、重命名、批量格式化，也不从旧 uCore 仓库复制代码。
- 修改实现前，阅读相关源码、头文件、构建脚本、测试和文档；尤其核对启动流程、页表、内存管理、陷阱/中断及特权级等体系结构约束。
- 修改后必须执行实际适用的编译或测试，才可声称完成；无法测试时应明确说明原因。
- 对 uCore 类项目，优先使用仓库支持的 `make`、`make qemu`、`make grade` 等命令；根据真实输出修复问题。
- 不随意重装、升级或替换既有工具链、QEMU、OpenSBI 等环境；环境问题应先诊断并说明。
